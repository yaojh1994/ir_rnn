...
def lstm_model(time_steps, rnn_layers, dense_layers=None):
    ...
    def lstm_cells(layers):
        if isinstance(layers[0], dict):
            return [tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(layer['steps'],\
                    state_is_tuple=True),layer['keep_prob']) if layer.get('keep_prob') \
                    else tf.nn.rnn_cell.BasicLSTMCell(layer['steps'],state_is_tuple=True)for layer in layers]
    ...
    def dnn_layers(input_layers, layers):
        if layers and isinstance(layers, dict):
            ...
            return tf.contrib.layers.dnn(input_layers, layers)
        else:
            return input_layers
